...
RSpec.describe Avo::Cards::CheckoutsChart do
  it "counts checkouts older than the selected range without crashing" do
    ...
    card = described_class.new(parent: Avo::Dashboards::Home.new)
    allow(card).to receive_messages(range: 30, current_user: admin)

    data = card.query.result_data
